...
from parameters import DETECTION_THRESH

THREAD_STOP = False
BLOB_PATH   = str((Path(__file__).parent / Path('../models/mobilenet-ssd_openvino_2021.4_5shave.blob')).resolve().absolute())
RUNNING     = False
...
class YoloCamera(BaseCamera):
    ...
    def setup(self):
        ...
        self._spatialDetectionNetwork = self._pipeline.create(dai.node.MobileNetSpatialDetectionNetwork)
        ...
        self._objectTracker = self._pipeline.create(dai.node.ObjectTracker)

        self._xoutRgb = self._pipeline.create(dai.node.XLinkOut)
        self._trackerOut = self._pipeline.create(dai.node.XLinkOut)

        self._xoutRgb.setStreamName("rgb")
        self._trackerOut.setStreamName("detections")

        # Properties
        self._camRgb.setPreviewSize(300, 300)
        ...
        self._objectTracker.setDetectionLabelsToTrack([7])  # track only car
        # possible tracking types: ZERO_TERM_COLOR_HISTOGRAM, ZERO_TERM_IMAGELESS, SHORT_TERM_IMAGELESS, SHORT_TERM_KCF
        self._objectTracker.setTrackerType(dai.TrackerType.ZERO_TERM_COLOR_HISTOGRAM)
        # take the smallest ID when new object is tracked, possible options: SMALLEST_ID, UNIQUE_ID
        self._objectTracker.setTrackerIdAssignmentPolicy(dai.TrackerIdAssignmentPolicy.SMALLEST_ID)
        ...
        self._objectTracker.passthroughTrackerFrame.link(self._xoutRgb.input)
        self._objectTracker.out.link(self._trackerOut.input)
