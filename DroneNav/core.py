...
from parameters import ALTITUDE, HEARTBEAT_TIMEOUT, ALTITUDE_FUZZINESS

from commands import setPositionTarget
from dronekit import Vehicle, VehicleMode
from cameraRelated.baseCamera import BaseCamera

from flightRelated.baseFlight import BaseFlight
from flightRelated.noDetection import NoDetection
from flightRelated.search import Search
from flightRelated.follow import Follow
from flightRelated.backoff import Backoff
...
class Core:
    ...
    def __init__(self, vehicle: Vehicle, camera: BaseCamera):
        ...
        # Setup flightRelated in heirarchy order
        self.flightRelated.append(Backoff(self.vehicle, self.camera))
        self.flightRelated.append(Follow(self.vehicle, self.camera))
        self.flightRelated.append(Search(self.vehicle, self.camera))
        self.flightRelated.append(NoDetection(self.vehicle, self.camera))
        ...
        while not self.state is ExecutionState.Stop:
            ...
            if self.state is ExecutionState.Init:
                ...
                if self.isReady() and self.isConnected() and self.isAltitudeOk():
                    # Prepare for running by resetting flightRelated
                    for rule in self.flightRelated:
                        rule.reset()
